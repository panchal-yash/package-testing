...
if [ $FILE_PLUGIN -eq 1 ]; then
  sudo sed -i '3i early-plugin-load=keyring_file.so' /etc/mysql/my.cnf
  sudo sed -i '4i keyring_file_data=keyring' /etc/mysql/my.cnf
elif [ $VAULT_PLUGIN -eq 1 ]; then
  sudo sed -i '3i early-plugin-load=keyring_vault.so'  /etc/mysql/my.cnf
  sudo sed -i '4i loose-keyring_vault_config=/home/mohit.joshi/pxc_scripts/vault/keyring_vault_pxc1.cnf'  /etc/mysql/my.cnf
fi
"""
fi
...
if [ $FILE_PLUGIN -eq 1 ]; then
  ...
  sudo sed -i '3i early-plugin-load=keyring_vault.so' /etc/mysql/my.cnf
  sudo sed -i '4i loose-keyring_vault_config=/home/mohit.joshi/pxc_scripts/vault/keyring_vault_pxc2.cnf' /etc/mysql/my.cnf
fi
"""
fi
...
if [ $FILE_PLUGIN -eq 1 ]; then
  ...
  sudo sed -i '3i early-plugin-load=keyring_vault.so' /etc/mysql/my.cnf
  sudo sed -i '4i loose-keyring_vault_config=/home/mohit.joshi/pxc_scripts/vault/keyring_vault_pxc3.cnf' /etc/mysql/my.cnf
fi
"""
fi
